...
import { getLatestDatasetQuality, getLatestPilotExperiment } from "@/lib/api";
import type { DatasetQuality, FactorKey, PilotExperiment, ViewKey } from "@/types/research";
...
function PilotDashboardView({ pilot }: { pilot: PilotExperiment | null }) {
  ...
  return <>
    <div className="page-heading"><div><div className="eyebrow">RESEARCH / PUBLIC-DATA PILOT</div><h1>Deterministic 2024 experiment</h1><p className="lede">Observed returns, stale-price sensitivity, and factor gates from validated NGX documents.</p></div><StatusBadge status="Completed with constraints" /></div>
    ...
    <section className="section-block"><SectionHeader eyebrow="MARKET PROXY" title="Equal-weight monthly returns" /><div className="panel detail-chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={chart}><CartesianGrid vertical={false} stroke="rgba(255,255,255,.08)" strokeDasharray="2 6" /><XAxis dataKey="month" tickLine={false} axisLine={false} tick={{ fill: "#8b989b", fontSize: 10 }} /><YAxis tickLine={false} axisLine={false} tick={{ fill: "#8b989b", fontSize: 10 }} tickFormatter={(value) => `${value}%`} /><Tooltip contentStyle={{ background: "#192326", border: "1px solid #334044", borderRadius: 8 }} formatter={(value: number) => [`${value.toFixed(2)}%`]} /><Line dataKey="marked" name="Marked-price proxy" stroke="#79e3ce" strokeWidth={2.5} dot={false} /><Line dataKey="official" name="Official-trade proxy" stroke="#d8b56a" strokeWidth={2} strokeDasharray="4 4" dot={false} /></LineChart></ResponsiveContainer></div></section>
...
function LiveFactorsView({ pilot }: { pilot: PilotExperiment | null }) {
  return <><div className="page-heading"><div><div className="eyebrow">RESEARCH / FACTOR GATES</div><h1>Factor eligibility</h1><p className="lede">A factor is calculated only when its observed inputs satisfy the declared gate.</p></div></div><div className="factor-selector">{pilot?.factor_eligibility.map((factor) => <div className="panel" key={factor.factor}><div className="detail-title"><h2>{factorLabels[factor.factor]}</h2><StatusBadge status={factor.status} /></div><div className="method-note"><BookOpen size={15} /><span>{factor.reasons.join(" · ")}</span></div></div>) ?? <div className="panel">Loading factor gates…</div>}</div></>;
}
...
export function ResearchConsole({ view }: { view: ViewKey }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><header className="topbar"><button className="icon-button menu-trigger" onClick={() => setSidebarOpen(true)} aria-label="Open navigation"><Menu size={19} /></button><div className="breadcrumb"><span>Workspace</span><span>/</span><b>{title}</b></div><div className="topbar-actions"><span className="demo-pill"><span /> Public-data pilot</span><button className="icon-button" aria-label="Help"><CircleHelp size={17} /></button><button className="icon-button" aria-label="Notifications"><Bell size={17} /><i className="notification-dot" /></button><div className="topbar-avatar">SJ</div></div></header><div className="page-body">{view === "dashboard" && <PilotDashboardView pilot={pilot} />}{view === "factors" && <LiveFactorsView pilot={pilot} />}{view === "regimes" && <BlockedResearchView title="Regime analysis unavailable" reason="One year of monthly observations is insufficient for a defensible multi-state regime model." />}{view === "portfolio" && <BlockedResearchView title="Composite portfolio unavailable" reason="Size, Value and Liquidity have not passed their data gates, so a composite portfolio would be misleading." />}{view === "experiments" && <PilotExperimentsView pilot={pilot} />}</div><footer className="site-footer"><span>NGX Research Console <i>·</i> academic research environment</span><span>Dataset <b>{quality?.dataset_id ?? "loading"}</b> <i>·</i> deterministic outputs only</span></footer></main>
