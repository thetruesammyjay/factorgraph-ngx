...
import type { FactorKey, ViewKey } from "@/types/research";
...
function DashboardView({ onNewExperiment }: { onNewExperiment: () => void }) {
  ...
  return <>
    ...
    <div className="hero-grid">
      ...
      <div className="research-rail">
        ...
        <div className="rail-foot"><span className="check-circle"><Check size={12} /></span><span>All validation gates passed</span></div>
      </div>
    </div>
...
export function ResearchConsole({ view }: { view: ViewKey }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}><div className="brand"><LogoMark /><button className="icon-button mobile-close" onClick={() => setSidebarOpen(false)}><X size={17} /></button></div><div className="workspace-switcher"><span className="workspace-avatar">SJ</span><div><b>Samuel Justin</b><small>Research workspace</small></div><ChevronDown size={15} /></div><nav><span className="nav-label">Workspace</span>{NAV_ITEMS.map((item) => { const Icon = iconMap[item.icon as keyof typeof iconMap]; const active = item.href === `/${view}` || (view === "dashboard" && item.href === "/dashboard"); return <a className={active ? "active" : ""} href={item.href} key={item.href} onClick={() => setSidebarOpen(false)}><Icon size={17} /><span>{item.label}</span>{item.label === "Experiments" && <span className="nav-count">3</span>}</a>; })}</nav><div className="sidebar-bottom"><div className="data-health"><span className="health-dot" /><div><b>Data pipeline healthy</b><small>Synced 18 min ago</small></div></div><a href="#docs"><BookOpen size={16} /> Documentation</a><a href="#settings"><Settings2 size={16} /> Workspace settings</a></div></aside>
    <main className="main-content"><header className="topbar"><button className="icon-button menu-trigger" onClick={() => setSidebarOpen(true)} aria-label="Open navigation"><Menu size={19} /></button><div className="breadcrumb"><span>Workspace</span><span>/</span><b>{title}</b></div><div className="topbar-actions"><span className="demo-pill"><span /> Demo data</span><button className="icon-button" aria-label="Help"><CircleHelp size={17} /></button><button className="icon-button" aria-label="Notifications"><Bell size={17} /><i className="notification-dot" /></button><div className="topbar-avatar">SJ</div></div></header><div className="page-body">{view === "dashboard" && <DashboardView onNewExperiment={handleNewExperiment} />}{view === "factors" && <FactorsView />}{view === "regimes" && <RegimesView />}{view === "portfolio" && <PortfolioView />}{view === "experiments" && <ExperimentsView onNewExperiment={handleNewExperiment} />}</div><footer className="site-footer"><span>NGX Research Console <i>·</i> academic research environment</span><span>Dataset <b>ngx_monthly_v3</b> <i>·</i> deterministic outputs only</span></footer></main>
